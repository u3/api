...
def call(model, protocol, prompt_path, out_json, max_tokens=24000, reasoning=None, label=''):
    cmd=[sys.executable, f'{S}/tools/zenmux_chat.py','--model',model,'--protocol',protocol,'--user',prompt_path,'--out',out_json,'--max-tokens',str(max_tokens),'--timeout','3000','--label',label,'--min-flows','60']
...
def main():
    ...
    if a.stage in ('synth','all'):
        ...
        pp=f'{P}/plan_final.prompt.md'; open(pp,'w').write(body); out,err,rc=call('anthropic/claude-opus-5','anthropic',pp,f'{P}/plan_final.json',max_tokens=40000,label='plan_final')
